...
def split_data(dataframe, predictors, target, test_size=0.1):
    ...
    something1 = dataframe[[predictors]]
    something2 = dataframe[target]
    X_train, X_test, y_train, y_test = sklearn.model_selection.train_test_split(something1, something2, test_size=0.1)
    return X_train, X_test, y_train, y_test


def single_feature_linear_regression(dataframe):
    # Fill in the function here
    X = dataframe[["fat-total"]]
    ...
    preds = model.predict(X_test)
    srcore = sklearn.metrics.r2_score(y_test, preds)
...
def multi_feature_linear_regression(dataframe):
    # Fill in the function here
    X = dataframe[["fat-total","sodium","carbohydrates","cholesterol"]]
    Y = dataframe["calories"]
    X_train, X_test, y_train, y_test = sklearn.model_selection.train_test_split(X, Y, test_size=0.1)
